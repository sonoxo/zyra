...
export async function blockchainRoutes(fastify) {
    ...
    fastify.get('/api/blockchain/monitor', async (request, reply) => {
        const { chain, address, alertThreshold } = request.query;
        if (!chain || !address) {
    ...
    fastify.post('/api/blockchain/webhook', async (request, reply) => {
        const { chain, address, alertTypes } = request.body;
        if (!chain || !address) {
        ...
        case 'eth':
            return {
                getTransactions: async (address, limit) => {
                    ...
                    const res = await fetch(`https://api.etherscan.io/api?module=account&action=txlist&address=${address}&startblock=0&endblock=99999999&page=1&offset=${limit}&sort=desc&apikey=${apiKey}`);
                ...
                getBalance: async (address) => {
                    if (!apiKey)
                        throw new Error('Missing ETHERSCAN_API_KEY');
                    const res = await fetch(`https://api.etherscan.io/api?module=account&action=balance&address=${address}&tag=latest&apikey=${apiKey}`);
                    const data = await res.json();
        ...
        case 'matic':
            return {
                getTransactions: async (address, limit) => {
                    ...
                    const res = await fetch(`https://api.polygonscan.com/api?module=account&action=txlist&address=${address}&startblock=0&endblock=99999999&page=1&offset=${limit}&sort=desc&apikey=${apiKey}`);
                    const data = await res.json();
                    return data.status === '1' ? data.result : [];
                },
                getBalance: async (address) => {
                    if (!apiKey)
                        throw new Error('Missing POLYGONSCAN_API_KEY');
                    const res = await fetch(`https://api.polygonscan.com/api?module=account&action=balance&address=${address}&tag=latest&apikey=${apiKey}`);
                    const data = await res.json();
        ...
        case 'binance':
            return {
                getTransactions: async (address, limit) => {
                    ...
                    const res = await fetch(`https://api.bscscan.com/api?module=account&action=txlist&address=${address}&startblock=0&endblock=99999999&page=1&offset=${limit}&sort=desc&apikey=${apiKey}`);
                    const data = await res.json();
                    return data.status === '1' ? data.result : [];
                },
                getBalance: async (address) => {
                    if (!apiKey)
                        throw new Error('Missing BSCSCAN_API_KEY');
                    const res = await fetch(`https://api.bscscan.com/api?module=account&action=balance&address=${address}&tag=latest&apikey=${apiKey}`);
                    const data = await res.json();
                    return data.status === '1' ? parseFloat(data.result) / 1e18 : 0;
                },
            };
...
function analyzeTransactions(transactions, threshold, chain) {
    const issues = [];
    let totalIn = 0;
    let totalOut = 0;
...
function calculateRiskScore(analysis, txPerMinute, balance) {
    let score = 0;

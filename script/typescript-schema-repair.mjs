...

{
  ...
  source = replaceOnce(source, '{session.summary && ((session.summary as any).findingsCount as number) > 0 && (', '{Boolean(session.summary) && ((session.summary as any).findingsCount as number) > 0 && (', "pentest unknown summary rendering");
...

{
  ...
  source = replaceOnce(source, '      const scannedRepoCount = new Set(allScans.map(s => s.repositoryId).filter(Boolean)).size;', '      const scannedRepoCount = new Set(allScans.filter(s => s.targetType === "repository").map(s => s.targetId).filter((id): id is string => Boolean(id))).size;', "scan target repository coverage");
